...
class signup : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        signup.setOnClickListener {
            ...
            if (!username.isNullOrBlank() && !email.isNullOrBlank() && !password.isNullOrBlank() && username.length >= 8 && username.length <= 11 && password.length >= 8 && email.contains("@")&& email.contains(".com") && email.length>=10) {
                ...
                database.child(username).get().addOnSuccessListener {
                    if (it.exists()) {
                        ...
                        database.child(username).setValue(user).addOnSuccessListener {
                            ...
                            one.show()
                            editor.putString("unamepref", username)
...
//
            ...
            }else if(!email.contains("@")&& !email.contains(".com") )
            {
                ...
                val one = make(
                    layout_id,
                    "The username max 11 characters and password min 8 characters",
                    LENGTH_SHORT
                )
                one.setTextColor((Color.parseColor("#B70404")))
                one.setBackgroundTint(Color.parseColor("#080202"))
